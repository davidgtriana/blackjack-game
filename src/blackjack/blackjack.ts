...
export class BlackjackGame {
    ...
    get dealer_hand(): Hand{ return this.active_hands[0]; }
    ...
    public print(){ this.active_hands.forEach(hand=>{hand.print()}); }
    ...
    public drawCardFromTheShoe(): Game.Card{
     ...
     */
    public placeBet(betbox_id:number, bet_amount:number) : Hand{
    ...
    public courseOfPlay() {
        ...
        if(GameConfig.DEBUG_MODE) console.log("Course of play...");
        this.playNextHand();
    }
    public payAndTake() {
    ...
    public async playNextHand(){
        ...
        if(this.have_all_hands_been_played()){
            if(GameConfig.DEBUG_MODE) console.log("All hands have been played.");
            this.print();
            this.dealerTurn();
            this.payAndTake();
            return;
        ...
        if(GameConfig.DEBUG_MODE) console.log("Current Hand Playing: " + this.current_active_hand);
        ...
        if (current_hand.isFinished){
            if (current_hand.isBlackJack) if(GameConfig.DEBUG_MODE) console.log("BlackJack...");
            this.finishHand();
            return;
        }
        ...
        if(GameConfig.DEBUG_MODE) console.log("Waiting for playing action...");
    }
    public have_all_hands_been_played() : boolean{
        return this.current_active_hand == this.active_hands.length;
    }
    public is_there_any_hand_still_on_the_table() : boolean{
        return this.active_hands.slice(1).some(hand => hand.isOnTheTable);
    }
    public is_any_hand_waiting_for_dealer_BJ(): boolean {
        return this.active_hands.some(hand => 
            hand.isOnTheTable && (hand.isBlackJack || hand.isDoubled || hand.isSplitted)
        );
    }
    public async dealCard(hand: Hand, msg?: string) {
        const card = this.drawCardFromTheShoe();
        hand.hit(card);

        let isDouble = false;
        if(msg && msg == "double") isDouble = true;
    ...
    public shouldKeepHandOnTable(hand: Hand): boolean {
    ...
    public async dealerTurn(){
        if (GameConfig.DEBUG_MODE) console.log("Dealer's turn...");

        if (this.is_there_any_hand_still_on_the_table()){
            if(this.dealer_hand.isTherePotentialForBJ()){
                if(this.is_any_hand_waiting_for_dealer_BJ()){
                    if(GameConfig.DEBUG_MODE) console.log("Dealer only plays one card");
                    await this.dealCard(this.dealer_hand);
            ...
            if(GameConfig.DEBUG_MODE) console.log("Dealer doesn't need to play");
        }        
    }
    /**
    ...
    public async playDealerHand() {
        console.log(this.dealer_hand.getHandTotal());

        // If the dealer's total exceeds 21, they bust and stop playing.
        if (this.dealer_hand.total > 21) {
            if (GameConfig.DEBUG_MODE) console.log("Dealer has Too Many...");
            return;
        }

        // If the dealer has a natural blackjack (21 with exactly 2 cards), they stop.
        if (this.dealer_hand.total == 21 && this.dealer_hand.cards.length == 2) {
            if (GameConfig.DEBUG_MODE) console.log("Dealer has a Blackjack...");
            return;
        }

        // If the dealer's total is greater than 17, they must stand.
        if (this.dealer_hand.total > 17) {
            this.dealer_hand.stand();
            if (GameConfig.DEBUG_MODE) console.log("Dealer stands on "+ this.dealer_hand.getHandTotal() + "...");
            return;
        }
        ...
        if (this.dealer_hand.total == 17) {
            ...
            if (this.dealer_hand.isSoft()) {
                ...
                if (GameConfig.IS_DEALER_HIT_ON_17) {
                    if (GameConfig.DEBUG_MODE) console.log("Dealer hits on soft 17...");
                    await this.dealCard(this.dealer_hand);
                    await this.playDealerHand(); // Recursive call to check if another hit is needed.
                    return;
                } else {
                    // Otherwise, the dealer stands on soft 17.
                    if (GameConfig.DEBUG_MODE) console.log("Dealer stands on soft 17...");
                    this.dealer_hand.stand();
                    return;
                }
            }
            // If it's a hard 17 (no Ace counted as 11), the dealer must stand.
            if (GameConfig.DEBUG_MODE) console.log("Dealer stands on hard 17...");
            return;
        }
        // If the dealer's total is below 17, they must hit.
        if (this.dealer_hand.total < 17) {
            if (GameConfig.DEBUG_MODE) console.log("Dealer hits on "+ this.dealer_hand.getHandTotal() +"...");
            await this.dealCard(this.dealer_hand);
            await this.playDealerHand(); // Recursive call to continue hitting until they stand or bust.
        }
    }
    public actionDeal(){
        if(this.state != GameState.Betting) return;
        // Check if at least one betbox has a hand
        if(!this.bet_boxes.some(betbox => betbox.hands.length > 0)) return;
        if(GameConfig.DEBUG_MODE) console.log("Initial Deal Out...");
        this.ui?.removeAllAvailableHighlight();
        this.initialDealOut();
    }
    public async actionHit(){
        if(!(this.state == GameState.CourseOfPlay || this.state == GameState.LastHand)) return;
    ...
    async actionDouble(){
        ...
        // Get the current hand
        const current_hand = this.active_hands[this.current_active_hand];
        if(!current_hand.canDouble) return;
    ...
    async actionSplit(){
        ...
        // Get the current hand
        const current_hand = this.active_hands[this.current_active_hand];
        if(!current_hand.canSplit) return;
    ...
    async actionSurrender(){
        if (GameConfig.DEBUG_MODE) console.log("Surrender button clicked...");
    }
    async actionInsurance(){
        if (GameConfig.DEBUG_MODE) console.log("Insurance button clicked...");
    }
    async actionUndo(){
        if (GameConfig.DEBUG_MODE) console.log("Undo button clicked...");
    }
    async actionClearBets(){
        if (GameConfig.DEBUG_MODE) console.log("Clear Bets button clicked...");
    }
    async actionResetTable(){
        if (GameConfig.DEBUG_MODE) console.log("Create reset button clicked...");
    }
    async actionCreateTable(){
        if (GameConfig.DEBUG_MODE) console.log("Create table button clicked...");
    }
    async actionNextHand(){
        if(!(this.state == GameState.PayAndTake)) return;
        this.startNewHand();
    }
